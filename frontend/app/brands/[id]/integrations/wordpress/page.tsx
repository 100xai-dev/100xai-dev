...
interface WordPressConfig {
  ...
  auth_type: "application_password" | "oauth";
...
const AUTH_STEPS = [
  { id: 1, title: "Site Information", description: "Enter your WordPress site details" },
  { id: 2, title: "Authentication", description: "Connect your WordPress account" },
  { id: 3, title: "Configuration", description: "Configure publishing settings" },
  { id: 4, title: "Test & Complete", description: "Test connection and finalize setup" }
];
...
export default function WordPressSetupPage() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setConfig({
          ...
          auth_type: integration.config.auth_type || "application_password",
          oauth_access_token: integration.config.oauth_access_token,
          oauth_refresh_token: integration.config.oauth_refresh_token,
          custom_post_type: integration.config.custom_post_type || "post",
          default_category: integration.config.default_category || "",
          default_tags: integration.config.default_tags || [],
          auto_publish: integration.config.auto_publish ?? true,
          featured_image_support: integration.config.featured_image_support ?? true
        });
  ...
  const validateStep = (step: number): boolean => {
    switch (step) {
      case 1:
        return !!(config.site_url && isValidUrl(config.site_url));
      case 2:
        if (config.auth_type === "application_password") {
          return !!(config.username && config.password);
        }
        return !!config.oauth_access_token;
      case 3:
        return !!config.custom_post_type;
      default:
        return true;
    }
  };
  ...
  const isValidUrl = (url: string): boolean => {
    ...
    try {
      // Normalize first (same logic as normalizeUrl) so bare domains like
      // "mysite.com" are accepted — the https:// prefix is added automatically.
  ...
  const testConnection = async () => {
    try {
      ...
      // Editing an existing integration: the password can't be reloaded for
      // security, so when the field is left blank fall back to the id-based
      // endpoint which tests against the stored credentials.
      const useStored = editMode && !config.password;

      const response = useStored
        ? await fetch(`/api/v1/brands/${brandId}/integrations/${editMode}/test`, {
            method: 'POST',
            headers: {
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900">
                {editMode ? "Edit WordPress Connection" : "Connect WordPress"}
              </h1>
              <p className="text-gray-600">
                Set up automatic publishing to your WordPress website
              </p>
            </div>
          </div>
        </div>
